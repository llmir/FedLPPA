    ...
    def _train(self, config):
        ...
        for i_iter in range(config['iters']):
            ...
            with autocast(enabled=self.amp):
                ...
                elif self.args.model == 'unet_univ3':
                    outputs, feature, de1, de2, de3, de4, prompts, fuse_feature, outputs_auxiliary = out
                else:
...
# TreeEnergyLoss
                ...
                if self.args.strategy in ['FedUniV2', 'FedUniV2.1']:
                    ...
                    pseudo_alpha = np.random.uniform(0.8, 1)
                    pseudo_logit = pseudo_alpha * outputs_soft + (1 - pseudo_alpha) * outputs_soft_auxiliary
                    outputs_soft_pseudo = torch.softmax(pseudo_logit, dim=1)
                    ...
                    pseudo_label = torch.argmax(pseudo_logit.detach(), dim=1)
                    ...
                    loss_auxi = torch.add(loss, loss_pls, alpha=self.args.beta)
                    loss = loss_auxi + 0.1 * out_gatedcrf
                    if i_iter > 0:
                        print(previous_prompts == prompts)
                        distance_prompts = l1_loss(previous_prompts.detach(), prompts.detach())
                    previous_prompts = prompts.detach()
        ...
        if self.args.strategy in ['FedUniV2', 'FedUniV2.1']:
            ...
            metrics_['client_{}_prompts'.format(self.cid)] = fl.common.ndarray_to_bytes(prompts.detach().cpu().numpy())
            if i_iter > 0:
...
def main():
    ...
    if args.strategy == 'FedRep':
        ...
        assert args.model in ['unet_univ2', 'unet_univ3']
