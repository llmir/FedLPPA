...
from flower_common_v4 import (BaseClient, MyModel, fit_metrics_aggregation_fn, evaluate, get_evaluate_fn,
...
# TreeEnergyLoss
                ...
                if self.args.strategy in ['FedUniV2', 'FedUniV2.1']:
                    ...
                    if self.current_iter <= self.warm_up_iter:
                        beta = self.args.beta * self.current_iter / self.warm_up_iter
                    else:
                        beta = self.args.beta
                    ...
                    #     beta = self.args.beta * (math.exp((self.current_iter / self.warm_up_iter) * 2 / 3) - 1) / (math.exp(2 / 3) - 1)
                    # else:
                    #     beta = self.args.beta
                        
                    loss = torch.add(loss, loss_pls, alpha=beta)
                    if i_iter > 0:
...
from flower_common_v4 import PretrainDataset
