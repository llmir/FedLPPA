    ...
    def _train(self, config):
        ...
        for i_iter in range(config['iters']):
            ...
            with autocast(enabled=self.amp):
                ...
                out_tree_loss, heatmaps = tree_loss(outputs, three_channel, high_feats, unlabeled_RoIs, self.args.tree_loss_weight)
                ...
                if self.args.strategy == 'FedAP' and (self.current_iter + 1) == self.args.iters:
        ...
        pseudos = torch.argmax(torch.softmax(heatmaps, dim=1), dim=1, keepdim=True)
        ...
        for i in range(self.args.num_classes):
            ...
            # print(heatmap.dtype, heatmap.size())
            metrics_['client_{}_Heatmap_{}'.format(self.cid, i)] = fl.common.ndarray_to_bytes(heatmap.cpu().numpy())
        metrics_['client_{}_Pseudo'.format(self.cid)] = fl.common.ndarray_to_bytes(pseudos.cpu().numpy())
        ...
        if self.args.strategy == 'FedAP' and self.current_iter == self.args.iters:
...
def pretrain_model(args, writer, worker_init_fn):
    ...
    model = net_factory(net_type=args.model, in_chns=args.in_chns, class_num=args.num_classes)
    model.cuda()
...
def main():
    ...
    parser.add_argument('--beta', type=float, default=0.5,
                        help='The hyper parameter for FedAN')
    ...
    parser.add_argument('--model_momentum', type=float, default=0.5,
                        help='The hyper parameter for FedAP')
    # client
    ...
    if args.role == 'server':
        ...
        train_image_metrics = ['Image', 'Prediction', 'GroundTruth', 'Pseudo'] + \
                                ['Heatmap_{}'.format(i) for i in range(args.num_classes)]
