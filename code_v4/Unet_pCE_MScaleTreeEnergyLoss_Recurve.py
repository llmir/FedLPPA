...
from flower_common import TreeEnergyLoss,MScaleAddTreeEnergyLoss, MScaleRecurveTreeEnergyLoss
...
def train(args, snapshot_path):
    ...
    model.train()
    optimizer = optim.AdamW(model.parameters(),lr= base_lr,betas=(0.9, 0.999),eps=1e-8,
    ...
    tree_loss = MScaleRecurveTreeEnergyLoss()
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with autocast(enabled=amp):
                ...
                if args.model == 'fcnet':
                    ...
                    outputs, feature, de1, de2, de3, de4, high_feats1, high_feats2, high_feats3= out
                else:
                ...
                out_tree_loss, heatmaps, heatmaps_2,heatmaps_3 = tree_loss(outputs, three_channel, high_feats1, high_feats2, high_feats3, unlabeled_RoIs, args.tree_loss_weight)
            ...
            if iter_num % 10 == 0:
                ...
                for i in range(args.num_classes):
                    ...
                    heatmap2 = heatmaps_2[1, i, :, :].detach().unsqueeze(0)
                    heatmap2 = (heatmap2 - heatmap2.min()) / (heatmap2.max() - heatmap2.min())
                    heatmap3 = heatmaps_3[1, i, :, :].detach().unsqueeze(0)
                    heatmap3 = (heatmap3 - heatmap3.min()) / (heatmap3.max() - heatmap3.min())
                    if args.img_class == 'odoc':
                        heatmap = heatmap.repeat(3, 1, 1)
                        heatmap2 = heatmap2.repeat(3, 1, 1)
                        heatmap3 = heatmap3.repeat(3, 1, 1)
                    heatmap_num.append(heatmap)
                    heatmap_num.append(heatmap2)
                    heatmap_num.append(heatmap3)
                ...
                if args.img_class == 'odoc':
                    writer.add_image('train/Heatedmap1_cup', heatmap_num[0], iter_num,dataformats='CHW')
                    writer.add_image('train/Heatedmap2_cup', heatmap_num[1], iter_num,dataformats='CHW')
                    writer.add_image('train/Heatedmap3_cup', heatmap_num[2], iter_num,dataformats='CHW')
                    writer.add_image('train/Heatedmap1_disc', heatmap_num[3], iter_num,dataformats='CHW')
                    writer.add_image('train/Heatedmap2_disc', heatmap_num[4], iter_num,dataformats='CHW')
                    writer.add_image('train/Heatedmap3_disc', heatmap_num[5], iter_num,dataformats='CHW')
                
                if args.img_class == 'faz':
                    writer.add_image('train/Heatedmap1', heatmap_num[0], iter_num,dataformats='CHW')
                    writer.add_image('train/Heatedmap2', heatmap_num[1], iter_num,dataformats='CHW')
                    writer.add_image('train/Heatedmap3', heatmap_num[1], iter_num,dataformats='CHW')
