...
parser.add_argument('--sup_type', type=str,
                    default='scribble', help='supervision label type(scr ; label ; scr_n ; keypoint ; block)')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # print("outputs_aux1=",outputs_aux1.shape)
            ...
            if iter_num > 0 and iter_num % 20 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    ...
                    with torch.no_grad():
                        ...
                        loss_ce1 = ce_loss(outputs, label_batch.long()).item()
                        loss_ce2 = ce_loss(outputs_aux1, label_batch.long()).item()
                        loss_val = 0.5 * (loss_ce1 + loss_ce2)
                ...
                for class_i in range(num_classes-1):
                    ...
                    writer.add_scalar('info/total_loss_val', loss_val.item(), iter_num)
                ...
                logging.info(
                    'iteration %d : mean_dice : %f mean_hd95 : %f : mean_recall : %f mean_precision : %f : mean_jc : %f mean_specificity : %f : mean_ravd : %f : loss_val : %f' % (iter_num, performance, mean_hd95, mean_recall, mean_precision, mean_jc, mean_specificity, mean_ravd,loss_val))
                model.train()
