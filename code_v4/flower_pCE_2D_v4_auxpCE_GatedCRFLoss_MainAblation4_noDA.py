...
from flower_common_v4_mainablation4_noDA import (BaseClient, MyModel, fit_metrics_aggregation_fn, TreeEnergyLoss, MScaleRecurveTreeEnergyLoss, evaluate, get_evaluate_fn,
    ...
    def _train(self, config):
        ...
        for i_iter in range(config['iters']):
            ...
            with autocast(enabled=self.amp):
                ...
                elif self.args.model == 'unet_univ5_ablation':
                    outputs, feature, de1, de2, de3, de4, outputs_auxiliary, = out
                else:
...
# TreeEnergyLoss
                ...
                if self.args.strategy in ['FedALA']:
                    outputs_soft_auxiliary = torch.softmax(outputs_auxiliary, dim=1)
                    # pseudo_alpha = 0.75 # np.random.random()
                    pseudo_alpha = np.random.uniform(0.8, 1)
                    pseudo_label = pseudo_alpha * outputs_soft.detach() + (1 - pseudo_alpha) * outputs_soft_auxiliary.detach()
...
def main():
    ...
    if args.strategy == 'FedRep':
        ...
        assert args.model in ['unet', 'unet_head', 'unet_multihead', 'unet_univ5_ablation']
