...
for i_batch, sampled_batch in enumerate(trainloader):
    ...
    # print(outputs.shape, outputs_auxiliary.shape, label_batch.shape)
    outputs_soft = torch.softmax(outputs, dim=1)
    loss_dice = dice_loss(outputs_soft, label_batch[:].long().unsqueeze(1))
    loss = loss_ce
    print(loss.item(), loss_dice.item())
