...
from dataloaders.dataset_ft import BaseDataSets, RandomGenerator
...
parser.add_argument('--img_size', type=int, default=384, help='h*w')
...
def train(args, snapshot_path):
    ...
    # if args.amp == 1:
    #         scaler = GradScaler()
    model = net_factory(args, net_type=args.model, in_chns=args.in_chns, class_num=num_classes)
    state_dict = torch.load('../model/polyp/Rebuttal_WSL_Setting_FedLC_alpha0.01_1/{}_async_unet_lc_auxi_best_model.pth'.format(args.client).replace("client","client_"),map_location="cuda:0")
    # model_state_dict = model.state_dict()
    model.load_state_dict(state_dict=state_dict, strict=False)
    model.cuda()
    
    # ignored_params = ['distribution_prompts', 'uni_prompt', 'attention','fuse']
    # for name, param in model.named_parameters():
    #     if name in ignored_params:
    #         param.requires_grad = False
    
    db_train = BaseDataSets(base_dir=args.root_path, split="train", transform=None, client=args.client, sup_type=args.sup_type, img_class= args.img_class)
    ...
    model.train()
    # optimizer = optim.AdamW(model.parameters(),lr=base_lr,betas=(0.9, 0.999),eps=1e-2,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            elif args.model == 'unet_lc_auxi':
                    outputs, feature, de1, de2, de3, de4, heatmaps, aux_output, outputs_auxiliary = out
            elif args.model == 'unet_univ2':
                outputs, feature, de1, de2, de3, de4, prompts, fuse_feature, outputs_auxiliary = out
            elif args.model == 'unet_univ5':
            ...
            # print('max', torch.max(volume_batch), torch.max(outputs), torch.max(label_batch[:].long()))
            ...
            if iter_num % 200 == 0:
                ...
                #     writer.add_image('train/Heatedmap2_Disc', heatmap_num[1], iter_num,dataformats='CHW')
                # if args.img_class == 'faz':
                #     writer.add_image('train/Heatedmap', heatmap_num[0], iter_num,dataformats='CHW')
