...
from utils.pytorch.pytorch_deeplab_v3_plus.DenseCRFLoss import DenseCRFLoss
...
parser.add_argument('--weight', type=float,  default=0.1, help='densecrf weight')
...
def train(args, snapshot_path):
    ...
    # print(model)
    db_train = BaseDataSets(base_dir=args.root_path, img_class=args.img_class,split="train", transform=transforms.Compose([
    ...
    densecrf_loss = DenseCRFLoss(weight=args.weight, sigma_rgb=15, sigma_xy=100, scale_factor=0.5)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if args.img_class == 'odoc':
                three_channel = sampled_batch['image']
            denormalized_image = three_channel * 255.
            
            croppings = (sampled_batch['label'] != args.num_classes).float()
            
            loss_ce = ce_loss(outputs, label_batch[:].long())

            out_densecrf = densecrf_loss(
                denormalized_image,
                outputs_soft,
                croppings).cuda()

            loss = loss_ce + out_densecrf
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if iter_num > 0 and iter_num % 20 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    ...
                    #     loss_ce_val = ce_loss(outputs_val, label_batch[:].long()).item()
