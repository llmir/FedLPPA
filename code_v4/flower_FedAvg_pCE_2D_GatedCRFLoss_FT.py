...
def train(args, snapshot_path):
    ...
    state_dict = torch.load('../model/odoc/WeaklySeg_pCE_GatedCRFLoss_feduniv2.1/{}_async_unet_univ2_best_model.pth'.format(args.client).replace("client","client_"),map_location="cuda:0")
    # model_state_dict = model.state_dict()
    model.load_state_dict(state_dict=state_dict, strict=False)
    model.cuda()
    db_train = BaseDataSets(base_dir=args.root_path, split="train", transform=None, client=args.client, sup_type=args.sup_type)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            logging.info(
                'iteration %d : loss : %f, loss_ce: %f, loss_gated: %f' %
                (iter_num, loss.item(), loss_ce.item(), out_gatedcrf.item()))
